# CLAUDE.md — Taskbar Inn 開発指示書 (auto mode)

このファイルは Claude Code が **auto mode で自走** するための環境・プロセスの正本です。
...
**Godot エディタは使えない。Claude Code はファイル生成・編集と CLI 実行しかできない。**
...
taskbar-inn/
  ...
  data/
    ...
    titles.json
  docs/
    SPEC.md                # この CLAUDE.md の要約 + 計算式の正本

...
- [x] 清掃/仕込みは対象が満たされたら自動で休憩へ戻る（汚れ0・在庫満タン）
- [x] 「手を止める」を「休憩」へ変更
...
- [x] 3. 収穫スキル・野菜在庫・汎用素材（Lv3の最小ループ）
  - [x] 収穫スキルと収穫作業カードを `skills.json` / skill_manager に実装
  - [x] 野菜在庫と食材保管庫/畑/野菜倉庫由来 cap を game_state / save に追加
  - [x] 汎用素材を basic/standard/good の補助素材として追加（入手は dev 仮ボタン）
  - [x] Lv3以降の cooking が vegetable/generic ingredient を参照して料理在庫を作る
  - [ ] 果実・魚介・特産品などを汎用素材カテゴリとして扱う方針を GAME_BALANCE に落とす

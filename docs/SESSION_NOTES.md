...
### 実装済み
- `SkillManager.get_effective_cooldown()` を追加。清掃用具・厨房・畑のLvで、対応する手動作業の所要時間を短縮する。
  道具は1回量、設備は所要時間という役割分担にした。スタッフ自動クールダウンは対象外。
- `InnSimulator`、`WorkStatusPanel`、`WorkPanel` は実効クールダウンを参照する。
  仕事詳細と作業状況カードに `所要 x.x秒` と、設備Lvによる短縮理由を表示する。
- `scripts/ui/kitchen_panel.gd` を追加。宿Lv2以降に左サイドバーへ `厨房` を表示する。
  左サイドバー順は `ホーム / 仕事 / 厨房 / 設備 / スタッフ / 道具 / ログ / 実績`。
- 厨房ページは操作ではなく詳細確認用。調理済み内訳、野菜/食材パック、現在作れる料理ランク、
  不足理由、客の要求料理ランク、調理ルールをまとめる。
- ホームは概要、厨房は料理/素材の詳細という役割分担にした。

### 注意 / 後続
- 食材パックは正式な街巡り/仕入れルート実装前の補助・テスト扱い。万能素材にはしない。
- 将来の収集・陳列・宿帳の専用ページ名は `展示`。置き土産/忘れ物/陳列/宿帳を扱い、
  パッシブ集客・売上・評判効果につなげる想定。
- 次の実装候補は `街巡り` と `展示MVP`。どちらも今回は未実装。

---

## 2026-06-15 追記 — 作業状況UI統合 / Lv2素材ループ

### 実装済み
- expanded の作業状況表示を `scripts/ui/work_status_panel.gd` に統合。
  現在作業、クールダウン進捗、清掃推奨/満杯/素材不足などの状況メッセージを1枚にまとめた。
- 下部の `作業:なし` 系ステータスバーを削除し、ログと作業状況が混ざらないよう整理。
- expanded サイズを 1180x760 に拡大し、ホームを資源バー、宿の状況グリッド、宿アップデート、最新ログへ整理。
- 宿アップデートの解放条件はボタン文言ではなく autowrap ラベルで表示し、条件が読めるようにした。
- UI表記を `料理在庫` → `調理済み`、`料理内訳` → `調理済み内訳`、
  `汎用素材` → `食材パック` に変更。内部名 `generic_ingredient_stock` は互換のため維持。
- 宿Lv2で `cooking` と同時に `harvesting`、`vegetable_stock`、`generic_ingredient_stock`、
  `field`、`vegetable_storage` を解放する仕様へ変更。
- `harvesting.tier_req` は 2。畑/野菜倉庫の unlock も `service_rank >= 2`。
- 調理は Lv2 以降、野菜または食材パックが無いと開始できない。調理済み満杯でも自動休憩。
- `GameState.cook_meals()` は調理済み保管容量の空きぶんだけ素材を消費するよう修正。
- オフライン中の料理人自動調理も `cook_meals()` 経由にし、素材を消費する。

### 注意
- 食材パックは basic/standard/good までの詰み防止・テスト補助。万能自動補完にはしない。
- Lv3 は収穫初解放ではなく、自給体制の拡張・安定化、農夫/自動収穫の前段階として扱う。

---

## 2026-06-15 追記 — スタッフページ分離 / スタッフ詰め所

### 実装済み
- 左サイドバー順を `ホーム / 仕事 / 設備 / ...` に変更。
- `data/skills.json` の `cooking` 表示名を「仕込み」から「調理」に変更。
- 「奉公人」表記をバッジ/称号/UI導線では「スタッフ」に変更。
- `data/upgrades.json` に `staff_room`（スタッフ詰め所）を追加。5人宿泊後に設備欄へ解放され、購入で
  `placement_slots +1`。
- スタッフ雇用/一覧を `scripts/ui/staff_panel.gd` に分離。スタッフ詰め所建設後、左サイドバーに
  `スタッフ` ボタンが出る。
- 仕事ページは作業カードとスキル詳細だけに整理。

### 注意
- 既存セーブ互換のため、すでにスタッフや配置枠がある場合もスタッフページを表示する。
- 5人宿泊チュートリアルは配置枠を直接増やさず、スタッフ詰め所の設備解放を通知する導線へ変更。
...
- Lv2 は調理と収穫を解放。料理は個別レシピではなく `meal_stocks` のランク別在庫
  (`basic` / `standard` / `good` / `luxury` / `specialty` / `vip`)。
- Lv2 から `harvesting` → `vegetable_stock` → `cooking` → `meal_stocks` の最小ループが動く。
- `food_stock` は削除禁止。旧セーブ互換のため、合計調理済み在庫として `meal_stocks` と同期する。
- 外部購入は万能な自動補完にしない。`generic_ingredient_stock` は UI 上「食材パック」。
  basic/standard/good までの補助素材で、現時点の入手は dev ボタン「食材パック+20」のみ。
- 倉庫の役割は分離済みの方向性:
  食材保管庫=未調理食材 cap、料理保管庫=調理後料理 cap、冷蔵庫=ロック定義中心で本効果は後続。
- 料理人は `tier_req:3` に加えて `action.cooking >= 20` で解放。農夫はまだ未実装。

### 実装済み
- 左サイドバー UI、道具ページ分離、もてなしカード、調理済みランク別在庫、収穫スキル、野菜/食材パック在庫。
- `GameState` は `meal_stocks` / `vegetable_stock` / `generic_ingredient_stock` を save/load する。
- `SkillManager` は清掃、もてなし、調理、収穫を処理する。
- Lv2以降の調理は野菜または食材パックを消費する。材料が無い場合は調理カードを開始できない。

...
## Visual MVP Step 1.7 実装メモ

2026-06-16 に、右ペイン背景+管理UIオーバーレイ案から `下段ステージ固定型` へ試作を進めた。

- 上段は `左サイドバー + 管理UI` に戻し、Home/仕事/厨房/設備/スタッフ/道具/ログ/実績を通常の管理領域で切り替える。
- `WorkStatusPanel` は上段管理領域の先頭にコンパクト表示する。
- 下段に `BottomInnStage` を常設し、宿屋背景、DEVマーカー、固定ルートを常に表示する。
- タブ操作中も下段ステージが消えないため、次にキャラ往復移動を入れても眺め続けられる。
- DEVマーカー/固定ルートは下段ステージ右側の `ルート` ボタンで ON/OFF できる。

この試作は「管理UIを操作していても、画面下段で宿屋のミニ生態系が動き続ける」体験の確認用。キャラ表示、往復移動、吹き出し、下段ステージの最終サイズ調整は次段階。
